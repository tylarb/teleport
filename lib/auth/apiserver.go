...
// tailscaleAuthRawResponse is returned when auth server validated callback
// parameters returned from Tailscale during OAuth2 flow
type tailscaleAuthRawResponse struct {
	// Username is authenticated teleport username
	Username string `json:"username"`
	// Identity contains validated OIDC identity
	Identity services.ExternalIdentity `json:"identity"`
...
/* authenticateTailscaleRequest validates Tailscale auth callback redirect

   POST /:version/tailscale/requests/authenticate

   Success response: tailscaleAuthRawResponse
*/
func (s *APIServer) authenticateTailscaleRequest(auth ClientI, w http.ResponseWriter, r *http.Request, p httprouter.Params, version string) (interface{}, error) {
	var req tailscaleAuthReq
	if err := httplib.ReadJSON(r, &req); err != nil {
		return nil, trace.Wrap(err)
	}
	response, err := auth.AuthenticateTailscaleRequest(req.Req)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	raw := tailscaleAuthRawResponse{
		Username: response.Username,
		Identity: response.Identity,
		Cert:     response.Cert,
		TLSCert:  response.TLSCert,
